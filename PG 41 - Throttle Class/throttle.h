...
namespace main_savitch_2A
{
	...
	public:
		...
		void shut_off() { position = 0; }	//Inline member function
		void shift(int amount);
		// CONSTANT MEMBER FUNCTIONS
		double flow() const { return position / double(top_position); }	//Inline member function
		bool is_on() const { return (position > 0); }	//Inline member function
	private:
		int top_position;
		int position;
	};

	bool operator <(const throttle& first, const throttle& second);
}
